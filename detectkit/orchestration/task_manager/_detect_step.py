...
class _DetectStepMixin(_TaskManagerBase):
    def _run_detect_step(
        ...
        for idx, detector_config in enumerate(config.detectors, 1):
            click.echo("  │")
            click.echo(
                f"  │ [{idx}/{len(config.detectors)}] "
                f"Detector: {detector_config.type}"
            )
            ...
            if not full_refresh and last_detection_ts:
                ...
                actual_from = (
                    max(actual_from, resume_from) if actual_from else resume_from
                )
            ...
            if start_time_str:
                ...
                actual_from = (
                    max(actual_from, start_time) if actual_from else start_time
                )
            ...
            while current_from < actual_to:
                ...
                if batch_results:
                    detection_data = {
                        ...
                        "is_anomaly": np.array(
                            [r.is_anomaly for r in batch_results], dtype=bool
                        ),
                        ...
                        "value": np.array(
                            [r.value for r in batch_results], dtype=np.float64
                        ),
