...
| **Internal dedup** | `ReplacingMergeTree` (version-collapse) | enforced PK + `ON CONFLICT` upsert | enforced PK + `ON DUPLICATE KEY UPDATE` |
...
- **[MySQL](./databases-mysql.md)** — database-based; requires MySQL 8.0+.
