...
class TestZScoreDetectorInit:
    """Test Z-Score detector initialization and validation."""
    ...
    def test_init_custom_params(self):
        ...
        detector = ZScoreDetector(threshold=2.5, window_size=50, min_samples=20)
    ...
    def test_validation_min_samples_exceeds_window(self):
        ...
        with pytest.raises(ValueError, match="min_samples cannot exceed window_size"):
            ZScoreDetector(window_size=50, min_samples=100)


class TestZScoreDetectorDetect:
    """Test Z-Score detector detection logic."""

    def test_detect_no_anomalies(self):
        """Test detection with no anomalies."""
        detector = ZScoreDetector(threshold=3.0, window_size=10, min_samples=5)
    ...
    def test_detect_window_size_limit(self):
        ...
        detector = ZScoreDetector(threshold=3.0, window_size=5, min_samples=3)
    ...
    def test_detect_severity_zscore(self):
        """Test that severity (Z-score) is calculated for anomalies."""
        detector = ZScoreDetector(threshold=3.0, window_size=10, min_samples=5)
        ...
        # Anomalous point should have severity (Z-score)
        anomaly_result = results[13]
        assert anomaly_result.is_anomaly == True
        assert "severity" in anomaly_result.detection_metadata
        assert anomaly_result.detection_metadata["severity"] > 3.0  # Should be high Z-score

    def test_detect_normal_distribution(self):
        """Test detection with normally distributed data."""
        np.random.seed(42)
        ...
        values[70] = -10.0  # 10 std away

        data = {
            "timestamp": np.array(
                [np.datetime64(f"2024-01-01T{i//60:02d}:{i%60:02d}:00", "ms") for i in range(100)]
            ),
            "value": values,
            "seasonality_data": np.array(["{}"] * 100),
            "seasonality_columns": [],
        }
        ...
        assert results[60].is_anomaly == True
        assert results[70].is_anomaly == True


class TestZScoreDetectorHashAndParams:
    """Test detector ID and parameter handling."""

    def test_get_detector_id_same_params(self):
        """Test that same params produce same ID."""
        detector1 = ZScoreDetector(threshold=2.5)
        detector2 = ZScoreDetector(threshold=2.5)
    ...
    def test_get_detector_id_different_params(self):
        ...
        detector1 = ZScoreDetector(threshold=2.5)
        detector2 = ZScoreDetector(threshold=3.0)

        assert detector1.get_detector_id() != detector2.get_detector_id()

    def test_get_detector_id_defaults(self):
        """Test that default params don't affect ID."""
        detector1 = ZScoreDetector()
        detector2 = ZScoreDetector(threshold=3.0, window_size=100, min_samples=30)

        # Both have all defaults, should be same
        assert detector1.get_detector_id() == detector2.get_detector_id()

    def test_get_detector_id_different_from_mad(self):
        """Test that ZScore and MAD with same params have different IDs."""
        ...
        assert zscore.get_detector_id() != mad.get_detector_id()
    ...
    def test_repr(self):
        ...
        detector = ZScoreDetector(threshold=2.5, window_size=50)
