...
self.onmessage = (e: { data: unknown }): void => {
  ...
  if (msg.type === 'run' && series) {
    ...
    // points the page must show for a band to appear. eff derives from it
    // directly (this is exactly effectiveStartIndex) — computing both via the
    // detector port would run the O(n) seasonality-cardinality scan twice per
    // knob change.
